        ...
        <tbody>
          <tr v-for="order in paginatedOrders" :key="order.id">
            ...
            <td class="actions">
              ...
              <button v-if="order.payment === 'Unpaid'" @click="markPaid(order)" class="btn-sm paid">Paid</button>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  setup() {
    ...
    const reload = () => { orders.value = getOrders() }
    onMounted(reload)
    ...
    const markPaid = (order) => {
      updateOrder(order.id, { payment: 'Paid' })
      reload()
    ...
    return {
      ...
      openStatusModal, closeStatusModal, submitStatusUpdate, markPaid
    }
  }
}
</script>
