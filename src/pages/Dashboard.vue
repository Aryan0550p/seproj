  ...
  <div class="dashboard">
    ...
    <div class="dashboard-grid">
      ...
      <div class="card alert-card">
        ...
        <p class="metric">{{ pendingPaymentStats.count }}</p>
        <span class="trend alert">₹{{ pendingPaymentStats.totalAmount.toLocaleString() }}</span>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const reload = async () => {
      ...
      pendingPaymentStats.value = await getPendingPaymentStats()
...
.card.alert-card::before {
  background: linear-gradient(90deg, #ff8b97, #dc3545);
}
