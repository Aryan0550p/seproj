...
import { getOrders, addOrder, updateOrder, getProducts, validateOrderStock } from '../store'
...
export default {
  ...
  setup() {
    ...
    const submitModal = async () => {
      ...
      // Validate stock before creating order (SCRUM-25)
      if (modalType.value === 'add') {
        ...
        const validation = await validateOrderStock([{
          productId: prod.id,
          qty: modalData.value.quantity
        }])

        if (!validation.isValid) {
          const issue = validation.issues[0]
          showToast(`Stock validation failed: ${issue.message}`, 'warning')
          return
        }
      }
      
      if (modalType.value === 'edit') {
        await updateOrder(modalData.value.id, {
    ...
    const submitStatusUpdate = async () => {
      ...
      if (updated?.isFinalized) {
        showToast(`Order ${selectedOrder.value.orderNumber} finalized successfully!`, 'success')
      } else {
