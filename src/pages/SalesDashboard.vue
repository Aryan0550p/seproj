  ...
  setup() {
    ...
    const shipOrder = async (order) => {
      try {
        ...
        showToast(error.message || 'Failed to ship order', 'warning')
      }
    }
    ...
    const submitModal = async () => {
      try {
        if (modalType.value === 'edit') {
          await updateOrder(modalData.value.id, { ...modalData.value })
        ...
        showToast(error.message || 'Failed to submit order', 'warning')
      }
    }
